...
<div class="flex flex-wrap items-center justify-between gap-4 mb-8 animate-slide-in">
    <div>
        <h3 class="flex items-center gap-3 text-3xl font-bold tracking-tight text-slate-900 dark:text-white">
            <span class="p-2 transition-all bg-blue-100 rounded-lg dark:bg-blue-500/20 shadow-glow-blue hover:scale-110">
    ...
    <div class="flex gap-3">
        <button onclick="openGenerateModal()" class="flex items-center gap-2 px-6 py-3 text-sm font-bold text-white transition-all shadow-lg rounded-xl bg-gradient-to-r from-blue-500 to-blue-700 shadow-glow-blue hover:shadow-xl hover:scale-105">
...
<div class="grid grid-cols-1 gap-6 mb-8 md:grid-cols-4 animate-slide-in" style="animation-delay: 100ms;">
    <div class="p-5 transition-all duration-300 bg-white border shadow-glass backdrop-blur-md rounded-2xl border-slate-200/50 dark:bg-slate-900/70 dark:border-slate-800 hover:-translate-y-1 hover:shadow-xl">
...
{{-- TABLE AREA --}}
<div class="overflow-hidden transition-all bg-white border shadow-glass backdrop-blur-md rounded-2xl border-slate-200/50 dark:bg-slate-900/70 dark:border-slate-800 animate-slide-in" style="animation-delay: 200ms;">
    <div class="flex flex-col gap-4 p-6 border-b md:flex-row md:items-center md:justify-between border-slate-200/50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20">
        <h4 class="text-lg font-bold text-slate-900 dark:text-white">Daftar Data QR Code</h4>
        <div class="flex gap-3">
            <div class="relative">
                <span class="absolute -translate-y-1/2 material-symbols-outlined text-[18px] left-3 top-1/2 text-slate-400">search</span>
                <input type="text" id="tableSearch" onkeyup="filterTable()" placeholder="Cari ID QR..." class="w-full pl-9 pr-4 py-2 text-sm bg-white dark:bg-slate-800 border rounded-xl border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-pertamina-blue/20 focus:border-pertamina-blue input-glass transition-all">
            </div>
        </div>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left">
            <thead class="bg-slate-50/50 dark:bg-slate-800/50">
                <tr>
                    <th class="px-6 py-4 text-xs font-bold tracking-wider uppercase text-slate-500 dark:text-slate-400">QR Code ID</th>
                    <th class="px-6 py-4 text-xs font-bold tracking-wider uppercase text-slate-500 dark:text-slate-400">SPBU</th>
                    <th class="px-6 py-4 text-xs font-bold tracking-wider uppercase text-slate-500 dark:text-slate-400">Jenis BBM</th>
                    <th class="px-6 py-4 text-xs font-bold tracking-wider uppercase text-slate-500 dark:text-slate-400">Kuota</th>
                    <th class="px-6 py-4 text-xs font-bold tracking-wider uppercase text-slate-500 dark:text-slate-400">Periode</th>
                    <th class="px-6 py-4 text-xs font-bold tracking-wider text-center uppercase text-slate-500 dark:text-slate-400">Status</th>
                    <th class="px-6 py-4 text-xs font-bold tracking-wider text-right uppercase text-slate-500 dark:text-slate-400">Aksi</th>
                </tr>
            </thead>
            <tbody id="qrTableBody" class="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                <tr class="transition table-row hover:bg-slate-50 dark:hover:bg-slate-800/50" id="row-{{ $index }}" 
                    ...
                    data-bbm="{{ $qr['bbm'] }}" data-kuota="{{ $qr['kuota'] }}" 
                    data-start="{{ $qr['start'] }}" data-end="{{ $qr['end'] }}" data-status="{{ $qr['status'] }}">
                    <td class="px-6 py-4">
                        <span class="font-mono text-xs font-bold text-slate-900 dark:text-white qrid-display">{{ $qr['id'] }}</span>
                    </td>
                    <td class="px-6 py-4 font-semibold text-slate-900 dark:text-white spbu-display">{{ $qr['spbu'] }}</td>
                    <td class="px-6 py-4">
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold bg-slate-100/50 text-slate-700 dark:bg-slate-800 dark:text-slate-300 rounded-lg border border-slate-200/50">
                            <span class="w-2 h-2 rounded-full {{ $qr['dot'] }}"></span>
                            <span class="bbm-display">{{ $qr['bbm'] }}</span>
                        </span>
                    </td>
                    <td class="px-6 py-4"><span class="font-bold text-slate-900 dark:text-white kuota-display">{{ $qr['kuota'] }}</span> Liter</td>
                    <td class="px-6 py-4 text-xs font-semibold text-slate-500 period-display">{{ \Carbon\Carbon::parse($qr['start'])->format('d M') }} - {{ \Carbon\Carbon::parse($qr['end'])->format('d M Y') }}</td>
                    <td class="px-6 py-4 text-center">
                        <span class="status-badge inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-full border {{ $qr['class'] }}">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <button onclick="viewQRDetail({{ $index }})" class="p-2 transition rounded-lg text-pertamina-blue hover:bg-pertamina-blue/10 dark:hover:bg-blue-900/20 dark:text-blue-400 group" title="Lihat">
                                <span class="text-lg transition-transform material-symbols-outlined group-hover:scale-110">visibility</span>
                            </button>
                            <button onclick="openEditModal({{ $index }})" class="p-2 transition rounded-lg text-orange-500 hover:bg-orange-500/10 dark:hover:bg-orange-900/20 dark:text-orange-400 group" title="Edit">
                                <span class="text-lg transition-transform material-symbols-outlined group-hover:scale-110">edit</span>
                            </button>
                            <button onclick="openDeleteModal({{ $index }})" class="p-2 transition rounded-lg text-pertamina-red hover:bg-pertamina-red/10 dark:hover:bg-red-900/20 dark:text-red-400 group" title="Hapus">
...
<div id="generateModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        <div class="fixed inset-0 transition-opacity bg-slate-900/75 backdrop-blur-sm" onclick="closeGenerateModal()"></div>
        <div class="relative w-full max-w-2xl overflow-hidden text-left align-bottom transition-all transform border border-white/50 dark:border-slate-700 bg-white/90 backdrop-blur-xl shadow-glass dark:bg-slate-900/90 rounded-2xl sm:my-8 sm:align-middle">
            <div class="px-6 py-5 border-b border-slate-200/50 dark:border-slate-800 bg-gradient-to-r from-blue-600/10 to-transparent">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="flex items-center justify-center w-12 h-12 shadow-md rounded-xl bg-gradient-to-br from-blue-500 to-blue-700">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-500 dark:text-slate-400">Buat QR Token baru untuk armada distribusi</p>
                        </div>
                    </div>
                    <button type="button" onclick="closeGenerateModal()" class="p-2 transition rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800">
            ...
            <div class="px-6 py-6">
                ...
                <form id="generateForm" class="space-y-4" onsubmit="event.preventDefault(); submitGenerateQR();">
                    <div class="p-6 text-center border-2 border-dashed rounded-xl border-blue-200 dark:border-blue-900 bg-blue-50/50 dark:bg-blue-900/10" id="qrVisualArea">
                        <div class="relative inline-block w-40 h-40 mx-auto transition-transform hover:scale-105" id="qrAnimationBox">
                            <div class="absolute inset-0 border-4 border-blue-500/30 rounded-xl"></div>
                            <!-- Scanner line -->
                            <div class="absolute top-0 left-0 w-full h-1 bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.8)] animate-pulse" id="scannerLine" style="animation: scan 2s infinite linear;"></div>
                    ...
                    <div id="genConfigFields">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">SPBU Target</label>
                                <select id="genSpbu" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 input-glass" required>
                                    <option value="">Pilih SPBU</option>
                                    <option>SPBU 34.123.01</option>
                                    <option>SPBU 34.121.05</option>
                                </select>
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Jenis BBM</label>
                                <select id="genBbm" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 input-glass" required>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" id="genKuota" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 input-glass" required />
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Durasi Validitas</label>
                                <select id="genDurasi" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 input-glass" required>
                    ...
                    <div id="genSuccessArea" class="hidden text-center">
                        <div class="inline-block p-4 mx-auto bg-white border border-blue-200 rounded-xl shadow-glass dark:bg-slate-100">
                            <!-- Real QR goes here -->
                            <div id="realQRCode" class="flex items-center justify-center w-32 h-32"></div>
                        </div>
                        <p class="mt-4 font-mono text-lg font-bold text-blue-600 dark:text-blue-400" id="genSuccessId"></p>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-200/50 dark:border-slate-800">
                        <button type="button" onclick="closeGenerateModal()" class="px-5 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 transition-all">
                            Tutup
                        </button>
                        <button type="submit" id="btnGen" class="px-5 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-blue-500 to-blue-700 rounded-xl hover:shadow-lg hover:scale-105 transition-all shadow-glow-blue">
        ...
        <div class="relative w-full max-w-2xl overflow-hidden text-left align-bottom transition-all transform border border-white/50 dark:border-slate-700 bg-white/90 backdrop-blur-xl shadow-glass dark:bg-slate-900/90 rounded-2xl sm:my-8 sm:align-middle">
            <div class="px-6 py-5 border-b border-slate-200/50 dark:border-slate-800 bg-gradient-to-r from-pertamina-blue/10 to-transparent">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-xl font-bold text-slate-900 dark:text-white" id="view-title">Detail QR Code</h3>
            ...
            <div class="px-6 py-6">
                <div class="grid grid-cols-2 gap-6">
                    <div class="flex flex-col items-center justify-center p-6 border border-dashed rounded-xl border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                        <!-- Simulate QR view -->
                        <div class="flex items-center justify-center p-3 bg-white border border-slate-200 size-40 rounded-xl shadow-sm">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p id="view-bbm" class="text-base font-semibold text-slate-900 dark:text-white">Pertalite</p>
                            </div>
                            <div>
                                <p class="mb-1 text-xs font-semibold uppercase text-slate-500">Kuota</p>
                                <p id="view-kuota" class="text-base font-bold text-slate-900 dark:text-white">8.000 L</p>
                            </div>
                        </div>
                        <div>
                            <p class="mb-1 text-xs font-semibold uppercase text-slate-500">Periode Aktif</p>
                            <p id="view-period" class="text-sm font-semibold text-slate-900 dark:text-white">07 Feb - 08 Feb</p>
                        </div>
                        <div>
                            <p class="mb-1 text-xs font-semibold uppercase text-slate-500">Status Terbaru</p>
                            <span id="view-status-container" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-bold rounded-full">
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t bg-slate-50/50 dark:bg-slate-800/50 border-slate-200/50 dark:border-slate-800">
                <button type="button" onclick="closeViewModal()" class="px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white border rounded-xl">Close</button>
            </div>
        </div>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div class="relative w-full max-w-lg overflow-hidden text-left align-bottom transition-all transform border border-white/50 dark:border-slate-700 bg-white/90 backdrop-blur-xl shadow-glass dark:bg-slate-900/90 rounded-2xl sm:my-8 sm:align-middle">
            <div class="px-6 py-5 border-b border-slate-200/50 dark:border-slate-800 bg-gradient-to-r from-orange-500/10 to-transparent">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="flex items-center justify-center w-12 h-12 shadow-md rounded-xl bg-gradient-to-br from-orange-500 to-amber-500">
                        ...
                        <div>
                            ...
                            <p id="edit-subtitle" class="text-sm font-mono text-orange-600 dark:text-orange-400">QR-ID</p>
                        </div>
                    </div>
            ...
            <div class="px-6 py-6">
                <form id="editForm" class="space-y-4" onsubmit="event.preventDefault(); submitEdit();">
                    <div>
                        <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Ubah Kuota (Liter)</label>
                        <input type="number" id="edit-kuota" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 focus:ring-2 focus:ring-orange-500/20 input-glass" required />
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Status Validasi</label>
                        <select id="edit-status" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 focus:ring-2 focus:ring-orange-500/20 input-glass" required>
                            <option>Aktif</option>
                            <option>Digunakan</option>
                            <option>Expired</option>
                        </select>
                    </div>
                    <div class="p-3 mt-4 text-xs font-medium border rounded-xl bg-orange-50 dark:bg-orange-500/10 text-orange-800 dark:text-orange-400 border-orange-200 dark:border-orange-500/30">
                        Informasi SPBU dan Jenis BBM tidak bisa diedit setelah di-generate. Silakan generate ulang jika ada kesalahan fatal.
                    </div>
                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-200/50">
                        <button type="button" onclick="closeEditModal()" class="px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white dark:bg-slate-800 border rounded-xl">Batal</button>
                        <button type="submit" class="px-5 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-orange-500 to-amber-600 rounded-xl shadow-lg hover:scale-105">Simpan Update</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div class="relative w-full max-w-sm overflow-hidden text-left align-bottom transition-all transform border shadow-2xl border-white/50 dark:border-slate-700 bg-white/90 backdrop-blur-xl dark:bg-slate-900/90 rounded-2xl sm:my-8 sm:align-middle">
            <div class="p-6 text-center">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">QR Code <strong id="delete-qr-id" class="text-pertamina-red"></strong> akan dihapus sementara dari sistem pemantauan.</p>
            </div>
            <div class="flex justify-center gap-3 px-6 py-4 border-t bg-slate-50/50 dark:bg-slate-800/50">
                <button type="button" onclick="closeDeleteModal()" class="px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white rounded-xl">Batal</button>
                <button type="button" id="confirmDeleteBtn" class="px-5 py-2.5 text-sm font-bold text-white bg-pertamina-red rounded-xl hover:shadow-lg hover:scale-105">Ya, Hapus</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function submitGenerateQR() {
        const bbm = document.getElementById('genBbm').value;
        const bcode = bbm.substring(0,3).toUpperCase();
        const rand = Math.floor(Math.random() * 900) + 100;
        const newId = `QR-${bcode}-2026-${rand}`;
        
        // Hide config, show success QR
        document.getElementById('genConfigFields').classList.add('hidden');
        document.getElementById('qrVisualArea').classList.add('hidden');
        document.getElementById('btnGen').classList.add('hidden');
        document.getElementById('genSuccessArea').classList.remove('hidden');
        document.getElementById('genSuccessId').textContent = newId;
        
        // Generate real QR inside the modal success view
        document.getElementById('realQRCode').innerHTML = '';
        QRCode.toCanvas(JSON.stringify({id: newId}), {width: 120, margin: 1, color: {dark: '#195de6', light: '#ffffff'}}, function (err, canvas) {
            if(!err) document.getElementById('realQRCode').appendChild(canvas);
        });

        const v = parseInt(document.getElementById('stat-total').innerText.replace('.',''));
        document.getElementById('stat-total').innerText = (v+1).toLocaleString('id-ID');
        showToast('Berhasil generate QR Code!');
    }
