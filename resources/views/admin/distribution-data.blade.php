...
<div class="flex flex-wrap items-center justify-between gap-4 mb-8 animate-slide-in">
    ...
    <div class="flex gap-3">
        <button onclick="exportData()" class="flex items-center gap-2 px-5 py-3 text-sm font-semibold transition-all bg-white border shadow-sm dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 hover:scale-105">
            <span class="text-xl material-symbols-outlined">download</span>
            Export Data
        </button>
        <button onclick="openAddModal()" class="flex items-center gap-2 px-5 py-3 text-sm font-bold text-white transition-all shadow-lg rounded-xl bg-gradient-to-r from-pertamina-blue to-blue-600 shadow-glow-blue hover:shadow-xl hover:scale-105">
...
{{-- FILTERS & SEARCH --}}
<div class="p-6 mb-6 transition-all border shadow-glass backdrop-blur-md bg-white/70 dark:bg-slate-900/70 border-white/50 dark:border-slate-800 rounded-2xl">
...
{{-- TABLE --}}
<div class="overflow-hidden transition-all border shadow-glass backdrop-blur-md bg-white/70 dark:bg-slate-900/70 border-white/50 dark:border-slate-800 rounded-2xl">
    ...
    <div class="flex items-center justify-between p-6 border-b border-slate-200/50 dark:border-slate-800">
        ...
        <div class="flex gap-2">
            <button onclick="refreshTable()" class="px-4 py-2 text-sm font-medium transition-all rounded-xl bg-slate-100/50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-slate-200/50 dark:divide-slate-700" id="distributionTable">
            ...
            {{-- TABLE BODY --}}
            <tbody class="divide-y bg-white/30 dark:bg-slate-900/30 divide-slate-100 dark:divide-slate-800" id="tableBody">
                @php
                    $fuelTypes = ['Pertalite', 'Pertamax', 'Solar', 'Dex'];
                    $statuses = [
                        ['name' => 'Completed', 'class' => 'bg-pertamina-green/10 text-pertamina-green dark:bg-pertamina-green/20 dark:text-green-400', 'icon' => 'check_circle'],
                        ['name' => 'In Transit', 'class' => 'bg-pertamina-blue/10 text-pertamina-blue dark:bg-pertamina-blue/20 dark:text-blue-400', 'icon' => 'local_shipping'],
                        ['name' => 'Loading', 'class' => 'bg-orange-500/10 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400', 'icon' => 'schedule']
                    ];
                @endphp
                ...
                @for ($i = 1; $i <= 10; $i++)
                    @php
                        ...
                        $sysDate = now()->subDays(rand(0, 30))->format('Y-m-d');
                        $displayDate = now()->subDays(rand(0, 30))->format('d M Y');
                        $time = now()->subDays(rand(0, 30))->format('H:i');
                        $truck = "TRK-00$i";
                        $driver = "Driver $i";
                        $destination = "SPBU 34.11{$i}03";
                        $location = "Jakarta Selatan";
                    @endphp
                    <tr class="transition table-row hover:bg-white/60 dark:hover:bg-slate-800/50"
                        id="row-{{ $i }}"
                        data-id="{{ $i }}"
                        data-truck="{{ $truck }}"
                        data-driver="{{ $driver }}"
                        data-fuel="{{ $fuel }}"
                        data-volume="{{ number_format($volume, 1) }}"
                        ...
                        data-statusicon="{{ $status['icon'] }}">

                        <td class="px-6 py-4">
                            <span class="text-sm font-semibold text-slate-900 dark:text-white row-number">{{ $i }}</span>
                        </td>

                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-gradient-to-br from-pertamina-blue/10 to-pertamina-blue/5">
                                    <span class="text-lg material-symbols-outlined text-pertamina-blue">local_shipping</span>
                                </div>
                                <div class="max-w-[120px]">
                                    <p class="font-mono text-sm font-bold truncate text-slate-900 dark:text-white truck-display">{{ $truck }}</p>
                        ...
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold bg-slate-100/50 text-slate-700 dark:bg-slate-800 dark:text-slate-300 rounded-lg">
                        ...
                        <td class="px-6 py-4">
                            <span class="text-sm font-bold text-slate-900 dark:text-white volume-display">{{ number_format($volume, 1) }}</span>
                            <span class="text-xs text-slate-500">KL</span>
                        </td>

                        <td class="px-6 py-4">
                            <div class="max-w-[160px]">
                                <p class="text-sm font-semibold truncate text-slate-900 dark:text-white destination-display">{{ $destination }}</p>
                        ...
                        <td class="px-6 py-4">
                            <div>
                                <p class="text-sm font-semibold text-slate-900 dark:text-white date-display">{{ $displayDate }}</p>
                                <p class="text-xs text-slate-500 time-display">{{ $time }} WIB</p>
                            </div>
                        </td>

                        <td class="px-6 py-4 text-center">
                            <span class="status-badge inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold border border-current/10 {{ $status['class'] }} rounded-full">
    ...
    {{-- PAGINATION --}}
    <div class="flex items-center justify-between px-6 py-4 border-t bg-slate-50/50 dark:bg-slate-800/50 border-slate-200/50 dark:border-slate-800">
        <p class="text-sm text-slate-600 dark:text-slate-400">
            Showing <span class="font-semibold text-slate-900 dark:text-white" id="showingCount">1-10</span> distributions
        </p>
        <div class="flex gap-2">
            ...
            <button class="px-4 py-2 text-sm font-semibold text-white transition-colors rounded-lg shadow-md bg-pertamina-blue hover:bg-blue-700 shadow-pertamina-blue/30 disabled:opacity-50">
                Next
            </button>
        </div>
    </div>
...
<div id="addModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div class="relative w-full max-w-2xl overflow-hidden text-left align-bottom transition-all transform border border-white/50 dark:border-slate-700 bg-white/90 backdrop-blur-xl shadow-glass dark:bg-slate-900/90 rounded-2xl sm:my-8 sm:align-middle">
            <div class="px-6 py-5 border-b border-slate-200/50 dark:border-slate-800 bg-gradient-to-r from-pertamina-blue/10 to-transparent">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" onclick="closeAddModal()" class="p-2 transition rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800">
                ...
                <form id="addForm" class="space-y-5" onsubmit="event.preventDefault(); submitAdd();">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" placeholder="e.g., TRK-001" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-pertamina-blue/20 focus:border-pertamina-blue input-glass" required />
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Driver Name</label>
                            <input type="text" placeholder="Driver name" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-pertamina-blue/20 focus:border-pertamina-blue input-glass" required />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-pertamina-blue/20 focus:border-pertamina-blue input-glass" required>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" placeholder="0.00" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-pertamina-blue/20 focus:border-pertamina-blue input-glass" required />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" placeholder="e.g., SPBU 34.11403" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-pertamina-blue/20 focus:border-pertamina-blue input-glass" required />
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Location</label>
                            <input type="text" placeholder="e.g., Jakarta Selatan" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-pertamina-blue/20 focus:border-pertamina-blue input-glass" required />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-pertamina-blue/20 focus:border-pertamina-blue input-glass" required />
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Time</label>
                            <input type="time" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-pertamina-blue/20 focus:border-pertamina-blue input-glass" required />
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-200/50 dark:border-slate-800">
                        <button type="button" onclick="closeAddModal()" class="px-5 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                            Cancel
                        </button>
                        <button type="submit" class="px-5 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-pertamina-blue to-blue-600 rounded-xl hover:shadow-lg hover:scale-105 transition-all shadow-glow-blue">
                            <span class="flex items-center gap-2">
...
<div id="viewModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div class="relative w-full max-w-3xl overflow-hidden text-left align-bottom transition-all transform border border-white/50 dark:border-slate-700 bg-white/90 backdrop-blur-xl shadow-glass dark:bg-slate-900/90 rounded-2xl sm:my-8 sm:align-middle">
            <div class="px-6 py-5 border-b border-slate-200/50 dark:border-slate-800 bg-gradient-to-r from-pertamina-blue/10 to-transparent">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-pertamina-blue to-blue-600 shadow-glow-blue">
                    ...
                    <button type="button" onclick="closeViewModal()" class="p-2 transition rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800">
                        <span class="material-symbols-outlined text-slate-500">close</span>
                    </button>
                </div>
            </div>
            ...
            <div class="px-6 py-6" id="viewModalLayout">
                <div class="grid grid-cols-2 gap-6">
                    <div class="space-y-4">
                        <div>
                            ...
                            <p id="view-truck" class="font-mono text-base font-bold text-slate-900 dark:text-white">TRK-001</p>
                        </div>
                        <div>
                            <p class="mb-1 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Driver Name</p>
                            <p id="view-driver" class="text-base font-semibold text-slate-900 dark:text-white">John Doe</p>
                        </div>
                        <div>
                            <p class="mb-1 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Fuel Type</p>
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 text-sm font-semibold bg-pertamina-blue/10 text-pertamina-blue border border-pertamina-blue/20 rounded-lg">
                                <span class="text-sm material-symbols-outlined">water_drop</span>
                        ...
                        <div>
                            ...
                            <p id="view-volume" class="text-base font-bold text-slate-900 dark:text-white">8.5 <span class="text-sm font-normal text-slate-500">KL</span></p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p id="view-destination" class="text-base font-semibold text-slate-900 dark:text-white">SPBU 34.11403</p>
                        </div>
                        <div>
                            <p class="mb-1 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Location</p>
                            <p id="view-location" class="text-base font-semibold text-slate-900 dark:text-white">Jakarta Selatan</p>
                        </div>
                        <div>
                            <p class="mb-1 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Date & Time</p>
                            <p id="view-datetime" class="text-base font-semibold text-slate-900 dark:text-white">08 Feb 2026, 14:30 WIB</p>
                        </div>
                        <div>
                            <p class="mb-1 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Status</p>
                            <span id="view-status-container" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-bold bg-pertamina-green/10 text-pertamina-green border border-pertamina-green/20 rounded-full">
                                <span class="text-sm material-symbols-outlined" id="view-status-icon">check_circle</span>
                ...
                {{-- Additional Info --}}
                <div class="p-4 mt-6 border bg-slate-50/50 dark:bg-slate-800/50 rounded-xl border-slate-200/50 dark:border-slate-700">
                    <p class="mb-2 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Additional Notes</p>
                    <p class="text-sm text-slate-600 dark:text-slate-300">Distribution logged automatically by the system. Verification matched with driver logs.</p>
                </div>
            </div>

            <div class="flex justify-end gap-3 px-6 py-4 border-t bg-slate-50/50 dark:bg-slate-800/50 border-slate-200/50 dark:border-slate-800">
                <button type="button" onclick="closeViewModal()" class="px-5 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                    Close
                </button>
                <button type="button" onclick="printDetails()" class="px-5 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-pertamina-blue to-blue-600 rounded-xl hover:shadow-lg hover:scale-105 transition-all shadow-glow-blue">
                    <span class="flex items-center gap-2">
...
<div id="editModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div class="relative w-full max-w-2xl overflow-hidden text-left align-bottom transition-all transform border border-white/50 dark:border-slate-700 bg-white/90 backdrop-blur-xl shadow-glass dark:bg-slate-900/90 rounded-2xl sm:my-8 sm:align-middle">
            <div class="px-6 py-5 border-b border-slate-200/50 dark:border-slate-800 bg-gradient-to-r from-orange-500/10 to-transparent">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="flex items-center justify-center w-12 h-12 shadow-md rounded-xl bg-gradient-to-br from-orange-500 to-orange-600">
                        ...
                        <div>
                            ...
                            <p id="edit-modal-subtitle" class="text-sm text-slate-500 dark:text-slate-400">Update information for TRK-XXX</p>
                        </div>
                    </div>
                    <button type="button" onclick="closeEditModal()" class="p-2 transition rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800">
                        <span class="material-symbols-outlined text-slate-500">close</span>
                    </button>
                </div>
            </div>
            ...
            <div class="px-6 py-6">
                <form id="editForm" class="space-y-5" onsubmit="event.preventDefault(); submitEdit();">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Truck ID</label>
                            <input type="text" id="edit-truck" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 input-glass" required />
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Driver Name</label>
                            <input type="text" id="edit-driver" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 input-glass" required />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Fuel Type</label>
                            <select id="edit-fuel" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 input-glass" required>
                                <option>Pertalite</option>
                                <option>Pertamax</option>
                                <option>Solar</option>
                                <option>Dex</option>
                        ...
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Volume (KL)</label>
                            <input type="number" id="edit-volume" step="0.01" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 input-glass" required />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Destination</label>
                            <input type="text" id="edit-destination" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 input-glass" required />
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Location</label>
                            <input type="text" id="edit-location" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 input-glass" required />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Date</label>
                            <input type="date" id="edit-date" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 input-glass" required />
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Time</label>
                            <input type="time" id="edit-time" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 input-glass" required />
                        </div>
                    </div>

                    <div>
                        <label class="block mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Status</label>
                        <select id="edit-status" class="w-full px-4 py-2.5 text-sm border rounded-xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 input-glass" required>
                            <option>Loading</option>
                            <option>In Transit</option>
                            <option>Completed</option>
                        </select>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-200/50 dark:border-slate-800">
                        <button type="button" onclick="closeEditModal()" class="px-5 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                            Cancel
                        </button>
                        <button type="submit" class="px-5 py-2.5 text-sm font-bold text-white transition-all shadow-lg rounded-xl bg-gradient-to-r from-orange-500 to-amber-600 hover:scale-105 shadow-orange-500/30">
                            <span class="flex items-center gap-2">
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        <div class="fixed inset-0 transition-opacity bg-slate-900/75 backdrop-blur-sm" onclick="closeDeleteModal()"></div>
        <div class="relative w-full max-w-sm overflow-hidden text-left align-bottom transition-all transform border shadow-2xl border-white/50 dark:border-slate-700 bg-white/90 backdrop-blur-xl dark:bg-slate-900/90 rounded-2xl sm:my-8 sm:align-middle">
            <div class="p-6 text-center">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">Peringatan: Data <strong id="delete-truck-id" class="text-slate-700 dark:text-slate-300"></strong> akan dihapus sementara secara visual dari tabel ini.</p>
            </div>
            <div class="flex justify-center gap-3 px-6 py-4 border-t bg-slate-50/50 dark:bg-slate-800/50 border-slate-200/50 dark:border-slate-800">
                <button type="button" onclick="closeDeleteModal()" class="px-5 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">Batal</button>
                <button type="button" id="confirmDeleteBtn" class="px-5 py-2.5 text-sm font-bold text-white transition-all rounded-xl hover:shadow-lg hover:scale-105 bg-pertamina-red shadow-pertamina-red/30">Ya, Hapus</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function updateRecordCount(count) {
        ...
        document.getElementById('showingCount').textContent = count > 0 ? "1-" + Math.min(count, 10) : "0";
    }
